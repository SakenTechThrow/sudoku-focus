  ...
  switch (status) {
    ...
    case 'empty':
      return {
        ...
        icon: Target,
        classes: 'border-cyan-200 bg-cyan-100/85 text-cyan-950 dark:border-cyan-300/20 dark:bg-cyan-400/10 dark:text-cyan-100',
      }
    case 'filled':
      return {
      ...
      {!isCollapsed ? (
        <>
          ...
          <div className="mt-4 flex items-center justify-between gap-3">
            <p className="text-sm font-medium text-slate-950 dark:text-slate-100">{confidence}</p>
        ...
        <div className="mt-3 rounded-[1.3rem] border border-slate-200 bg-slate-50/90 px-4 py-3 text-sm text-slate-600 dark:border-white/10 dark:bg-slate-950/45 dark:text-slate-300">
          Open the coach when you want help reading candidates or checking a move.
        </div>
      )}
    </section>
  )
